...
# --- 拼字挑戰 ---
    ...
    if st.session_state.get('success_trigger', False):
        st.balloons()
        st.success(f"🎯 答對了！正確答案：{current_word}")
        time.sleep(1.2)
    ...
    with st.container():
        ...
        # 核心防禦：Password 框
        d_key = f"spell_{st.session_state.score}_{st.session_state.idx}"
        with st.form(key=f"form_{d_key}", clear_on_submit=True):
            user_input = st.text_input("Spell", type="password", label_visibility="collapsed", placeholder="請拼出單字...", key=d_key).strip()
