...
# --- 挑戰頁 (錯題循環) ---
    ...
    with st.container():
        ...
        spk_key = f"spk_{st.session_state.idx}_{len(st.session_state.remaining_indices)}"
        if st.button("🔊 播放發音", key=spk_key):
        ...
        with st.form(key=f"form_{d_key}", clear_on_submit=True):
            user_input = st.text_input("拼寫單字", label_visibility="collapsed", placeholder="在此輸入單字...", key=f"ti_{d_key}").strip()
