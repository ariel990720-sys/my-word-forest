...
st.markdown("""
    ...
    .combo-text { color: #E64A19; font-size: 2rem; font-weight: bold; text-align: center; }
    .timer-text { font-size: 1.2rem; color: #555; text-align: right; }
    </style>
...
# --- 挑戰頁 (計時 + 錯題循環 + Combo) ---
    ...
    if st.session_state.combo > 1:
        st.markdown(f'<p class="combo-text">🔥 Combo {st.session_state.combo} !!</p>', unsafe_allow_html=True)
    ...
    if st.session_state.get('success_trigger', False):
        ...
        if not st.session_state.remaining_indices:
            st.balloons()
            st.info(f"🎊 單元完成！總耗時：{int(time.time() - st.session_state.start_time)} 秒")
            time.sleep(2)
    ...
    with st.container():
        ...
        if st.button("🔊 播放", key=f"spk_{st.session_state.idx}"):
            text_to_speech(current_word)

        with st.form(key=f"f_{st.session_state.idx}", clear_on_submit=True):
            user_input = st.text_input("輸入拼寫", label_visibility="collapsed", placeholder="請在此輸入單字...").strip()
            if st.form_submit_button("檢查答案"):
                if user_input.lower() == current_word.lower():
                    ...
                    st.session_state.combo = 0 # 斷 Combo
                    st.session_state.remaining_indices.append(st.session_state.idx) # 錯題循環
                    time.sleep(1.5)
                    st.session_state.idx = st.session_state.remaining_indices.pop(0)
                    st.rerun()
