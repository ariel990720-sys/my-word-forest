...
# --- 拼字挑戰 ---
    ...
    if st.session_state.get('success_trigger', False):
        st.success(f"🎯 答對了！答案是：{current_word}")
        time.sleep(1)
    ...
    with st.container():
        ...
        # 🔊 終極解決方案：直接顯示播放器控制條
        st.write("🔊 點擊下方播放鍵聽發音：")
        word_url = urllib.parse.quote(current_word)
        tts_url = f"https://translate.google.com/translate_tts?ie=UTF-8&q={word_url}&tl=en&client=tw-ob"
        st.audio(tts_url, format="audio/mpeg") # 這次不寫程式控制，直接讓你點
        ...
        with st.form(key=f"form_{d_key}", clear_on_submit=True):
            user_input = st.text_input("請拼出單字", label_visibility="collapsed", placeholder="請拼出單字...", key=d_key).strip()
