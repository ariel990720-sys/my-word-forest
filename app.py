...
# 終極發音方案：嵌入原生 HTML5 Audio 播放器
def text_to_speech(text):
    word = urllib.parse.quote(text.strip())
    tts_url = f"https://translate.google.com/translate_tts?ie=UTF-8&q={word}&tl=en&client=tw-ob"
    # 使用 st.audio 直接調用 Streamlit 原生組件，這是最穩定的方式
...
# --- 拼字挑戰 ---
    ...
    with st.container():
        ...
        c_ipa_val, c_play_btn = st.columns([2, 1])
        with c_ipa_val:
            if st.session_state.show_ipa: st.write(f"🎧 音標：{row['音標']}")
            else: st.write("🎧 音標已隱藏")
        
        with c_play_btn:
            play_clicked = st.button("🔊 播放音檔", key=f"btn_{st.session_state.idx}")
        ...
        with st.form(key=f"form_{d_key}", clear_on_submit=True):
            user_input = st.text_input("拼寫單字", label_visibility="collapsed", placeholder="請拼出單字...", key=d_key).strip()
